...
resetGame();
    ...
    // Ensure buttons don't submit forms in odd embeddings
    if (startBtn) startBtn.setAttribute('type','button');
    if (playAgain) playAgain.setAttribute('type','button');
    if (viewBreakdown) viewBreakdown.setAttribute('type','button');
    if (document.getElementById('btnHelp')) document.getElementById('btnHelp').setAttribute('type','button');

    // Indicate JS is live
    var jsStatus = document.getElementById('jsStatus');
    if (jsStatus) jsStatus.classList.add('hidden');
    console.log('%cJenga demo ready','padding:4px 8px;background:#2ea8ff;color:#000;border-radius:6px');
  } catch (err) {
    console.error('Boot error:', err);
    var status = document.getElementById('jsStatus');
    if (status) status.textContent = 'Error: ' + (err && err.message ? err.message : 'Unable to start');
  }
});
